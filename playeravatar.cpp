...
PlayerAvatar::PlayerAvatar(int n,QWidget *parent) : QWidget(parent)
{
    ...
    player->move(30+((n+4)%4)*430,20+((n+1)/5)*720);//设置位置
...
void PlayerAvatar::playerDie()//显示死亡标识
{
    voting->hide();
    badge->hide();
...
void PlayerAvatar::playerHideBadge()//显示警徽
{
    badge->hide();
}

void PlayerAvatar::playerShowBadge()//隐藏警徽
{
    badge->show();
}

void PlayerAvatar::playerShowIdentity(int n)//显示身份
{
    switch (n) {
    case 0:
    {
