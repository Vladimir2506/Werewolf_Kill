...
MyTimer::MyTimer(timerType type,QWidget *parent) : QWidget(parent)
{
    ...
    connect(timer,&QTimer::timeout,this,&MyTimer::timerUpdate);//每过一秒更新一次显示
    timer->start(1000);
...
void MyTimer::resetTimer(timerType type)//根据计时器类型确定初始秒数
{
...
void MyTimer::stop()//停止计时
{
    disconnect(timer,&QTimer::timeout,this,&MyTimer::timerUpdate);//断开更新时间的槽
    disconnect(this,SIGNAL(timeOut()),this->parent(),SLOT(timeUp()));//断开反馈时间到信息的槽
    timeShow->hide();//隐藏计时器
}
